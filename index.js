...
function userAllowed(interaction) {
  ...
  return TRADER_ROLES.some(r => interaction.member.roles?.cache?.has(r));
}

// ===== Helpers =====
function fmtPct(p){ if(p==null) return null; const n=Number(p); if(!isFinite(n)) return null; return Math.max(0,Math.min(100,Math.round(n))); }
function code(x){ return `\`${x}\``; }
function normPrice(str){ if(!str) return str; const s=String(str).replace(/[, ]/g,''); if(!/^\d+(\.\d+)?$/.test(s)) return str; return Number(s).toLocaleString('en-US',{maximumFractionDigits:8}); }
function statusEmoji(s){ return s==='Running'?'🚀':s==='BE'?'🟨':s==='Invalid'?'❌':s==='Closed'?'✅':'🟢'; }
function directionEmoji(d){ return d?.toLowerCase()==='long'?'🟢':'🔴'; }

function buildMessageContent(s, appendMention=false){
  ...
  if (appendMention && MENTION_ROLE_ID && MENTION_ROLE_ID.trim()) lines.push(`<@&${MENTION_ROLE_ID}>`);
  return lines.join('\n');
}
  ...
  try {
    ...
    if (interaction.isButton()) {
      ...
      if (action === 'close') {
        ...
        return interaction.showModal(modal.addComponents(new ActionRowBuilder().addComponents(resInput), new ActionRowBuilder().addComponents(rInput)));
      }
