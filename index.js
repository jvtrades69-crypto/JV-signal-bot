...
// - TP plans + auto-exec
// - Two-button update flow (+ select menu)
// - HARD PURGE summary (with try/catch)
// - Fixed: "Unknown interaction (10062)" by showing modals directly
...
function buildTitleChip(signal) {
  ...
  if (s.status === STATUS.STOPPED_OUT) return { show: true, text: `Loss -${Math.abs(rr).toFixed(2)}R` };
  if (s.status === STATUS.STOPPED_BE) {
    const anyFill = (s.fills || []).length > 0;
    return { show: true, text: anyFill ? `Win +${rr.toFixed(2)}R` : 'Breakeven' };
  }
  if (s.status === STATUS.CLOSED) return { show: true, text: `Win +${rr.toFixed(2)}R` };
  if ((s.fills || []).length > 0) return { show: true, text: `Win +${rr.toFixed(2)}R so far` };
...
function buildMentions(defaultRoleId, extraRoleRaw) {
  ...
  return { content: ids.map(id => `<@&${id}>`).join(' '), allowedMentions: { parse: [], roles: ids } };
}
...
async function updateSummary() {
  try {
    ...
    await channel.send({ content: text }).catch(e => console.error('summary send failed:', e));
  } catch (e) {
...
function controlRows(signalId) {
  ...
  const row2 = new ActionRowBuilder().addComponents(
    ...
    new ButtonBuilder().setCustomId(`updlevels_${signalId}`).setLabel('✏️ Update Levels').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId(`updmore_${signalId}`).setLabel('⚙️ More Updates').setStyle(ButtonStyle.Secondary),
  );
...
function makeUpdateLevelsModal(id) {
  const modal = new ModalBuilder().setCustomId(`modal_updlevels_${id}`).setTitle('Update Levels (Entry/SL/TP1–TP3)');
  const fields = [
...
function makeTp45Modal(id) {
  const modal = new ModalBuilder().setCustomId(`modal_tp45_${id}`).setTitle('Update TP4 & TP5');
...
function makePlansModal(id) {
  const modal = new ModalBuilder().setCustomId(`modal_plans_${id}`).setTitle('Update TP % Plans (0–100)');
...
function makeMetaModal(id) {
  const modal = new ModalBuilder().setCustomId(`modal_meta_${id}`).setTitle('Update Meta');
  const fields = [
...
client.on('interactionCreate', async (interaction) => {
  try {
    ...
    // Update Levels (Entry/SL/TP1–TP3)
    ...
    if (interaction.isButton()) {
      ...
      if (action === 'updlevels') return interaction.showModal(makeUpdateLevelsModal(id));
      if (action === 'updmore')   return interaction.reply({ content: 'Choose what to update:', components: [makeMoreUpdatesMenu(id)], ephemeral: true });
    ...
    // More Updates menu — **direct showModal**, no update/followUp first
    if (interaction.isStringSelectMenu() && interaction.customId.startsWith('menu_more_')) {
      ...
      if (choice === 'more_tp45')  return interaction.showModal(makeTp45Modal(id));
      if (choice === 'more_plans') return interaction.showModal(makePlansModal(id));
      if (choice === 'more_meta')  return interaction.showModal(makeMetaModal(id));
